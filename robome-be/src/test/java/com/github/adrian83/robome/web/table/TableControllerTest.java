...
public class TableControllerTest extends JUnitRouteTest {
    ...
    private TableController tableController = new TableController(tableServiceMock, responseMock, securityMock);
	...
	// when
	appRoute.run(HttpRequest
		.OPTIONS("/users/" + UUID.randomUUID().toString() + "/tables/" + UUID.randomUUID().toString() + "/"))
		.assertStatusCode(200).assertEntity("Fragments of imagination");
    }
    //
    // @Override
