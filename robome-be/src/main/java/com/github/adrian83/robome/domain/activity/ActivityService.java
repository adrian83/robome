...
public class ActivityService {
	...
	var entity = new ActivityEntity(ActivityKey.basedOnStageKey(req.stageKey()), req.name(), ActivityState.ACTIVE,
		Time.utcNow(), Time.utcNow());

	return Source.single(entity).via(activityRepository.saveActivity()).map(this::toActivity).runWith(Sink.head(),
		actorSystem);
    }

    public CompletionStage<Activity> updateActivity(UpdateActivityRequest req) {
	var entity = new ActivityEntity(req.activityKey(), req.name(), ActivityState.ACTIVE, Time.utcNow(),
		Time.utcNow());

	return Source.single(entity).via(activityRepository.updateActivity()).map(this::toActivity).runWith(Sink.head(),
		actorSystem);
    }

    public CompletionStage<ActivityKey> deleteActivity(DeleteActivityRequest req) {
	return Source.single(req.activityKey()).via(activityRepository.deleteActivity(req.activityKey().userId()))
		.runWith(Sink.head(), actorSystem);
    }

    public CompletionStage<Optional<Activity>> getActivity(GetActivityRequest req) {
	return activityRepository.getById(req.activityKey()).map(this::toActivity).runWith(Sink.headOption(),
		actorSystem);
    }

    public CompletionStage<List<Activity>> getStageActivities(ListStageActivitiesRequest req) {
	return activityRepository.getStageActivities(req.stageKey()).map(this::toActivity).runWith(Sink.seq(),
		actorSystem);
    }
