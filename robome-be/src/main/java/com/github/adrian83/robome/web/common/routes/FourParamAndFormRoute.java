...
public class FourParamAndFormRoute<T> extends AbsFormRoute<T> implements Supplier<Route> {
    ...
    private final PentaFunction<String, String, String, String, Class<T>, Route> action;

    public FourParamAndFormRoute(String[] path, Class<T> clazz, PentaFunction<String, String, String, String, Class<T>, Route> action) {
        super(path, clazz);
        this.action = action;
    }
    ...
    public Route get() {
        ...
        if (startsWithParameter()) {
            ...
            return pathPrefix(segment(), var1 -> new ThreeParamsAndFormRoute<T>(newPath, getClazz(), newFunc.apply(var1)).get());
        }

        return pathPrefix(pathHead(), new FourParamAndFormRoute<>(newPath, getClazz(), action));
    }
}
