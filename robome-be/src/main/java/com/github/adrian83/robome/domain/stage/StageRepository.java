...
public class StageRepository {
    ...
    private static final String SELECT_STAGE_BY_ID_STMT = "SELECT * FROM robome.stages WHERE table_id = ? AND stage_id = ? AND user_id = ? ALLOW FILTERING";
    private static final String SELECT_STAGES_BY_TABLE_ID_STMT = "SELECT * FROM robome.stages WHERE table_id = ? AND user_id = ? ALLOW FILTERING";
    private static final String UPDATE_STMT = "UPDATE robome.stages SET title = ?, state = ?, modified_at = ? WHERE table_id = ? AND stage_id = ? AND user_id = ?";
    ...
    public Flow<StageEntity, StageEntity, NotUsed> saveStage() {
        Function2<StageEntity, PreparedStatement, BoundStatement> statementBinder = (stage, prepStmt) -> prepStmt.bind(
                stage.key().stageId(), stage.key().tableId(), stage.key().userId(), stage.title(), stage.state().name(),
                toInstant(stage.createdAt()), toInstant(stage.modifiedAt()));
    ...
    public Flow<StageEntity, StageEntity, NotUsed> updateStage() {
        Function2<StageEntity, PreparedStatement, BoundStatement> statementBinder = (stage, prepStmt) -> prepStmt.bind(
                stage.title(), stage.state().name(), toInstant(stage.modifiedAt()), stage.key().tableId(),
                stage.key().stageId(), stage.key().userId());
    ...
    public Flow<StageKey, StageKey, NotUsed> deleteStage() {
        Function2<StageKey, PreparedStatement, BoundStatement> statementBinder = (key, prepStmt) -> prepStmt.bind(key.tableId(), key.stageId(), key.userId());
    ...
    public Source<StageEntity, NotUsed> getById(StageKey stageKey) {
        Statement<?> stmt = SimpleStatement.newInstance(SELECT_STAGE_BY_ID_STMT, stageKey.tableId(), stageKey.stageId(), stageKey.userId());
        return CassandraSource.create(session, stmt).map(this::fromRow);
    }

    public Source<StageEntity, NotUsed> getTableStages(TableKey key) {
        Statement<?> stmt = SimpleStatement.newInstance(SELECT_STAGES_BY_TABLE_ID_STMT, key.tableId(), key.userId());
        return CassandraSource.create(session, stmt).map(this::fromRow);
    }

    private StageEntity fromRow(Row row) {
        var key = new StageKey(row.get(USER_ID_COL, UUID.class), row.get(TABLE_ID_COL, UUID.class), row.get(STAGE_ID_COL, UUID.class));
        return new StageEntity(key, row.getString(TITLE_COL), valueOf(row.getString(STATE_COL)),
