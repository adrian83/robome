...
public class Security extends AllDirectives {
  ...
  public <T, P, R, S> Route secured(T p1, P p2, R p3, Class<S> clazz, Parameter3WithBodyRequest<T, P, R, S> logic) {
    Function<UserData, Route> userToRoute = userData -> entity(unmarshaller(clazz), form -> handleExceptions(logic.apply(userData, p1, p2, p3, form)));
    return withUserFromAuthHeader(userToRoute);
  }

 
  public <T> Route secured(Class<T> clazz, Parameter1Request<T> logic) {
    Function<UserData, Route> userToRoute = userData -> entity(unmarshaller(clazz), form -> handleExceptions(logic.apply(userData, form)));
    return withUserFromAuthHeader(userToRoute);
  }

  
  public <T, P> Route secured(P param, Class<T> clazz, Parameter2Request<P, T> logic) {
    Function<UserData, Route> userToRoute = userData -> entity(unmarshaller(clazz), form -> handleExceptions(logic.apply(userData, param, form)));
    return withUserFromAuthHeader(userToRoute);
  }

  
  public <P, R, S> Route secured(P p1, R p2, Class<S> clazz, Parameter2WithBodyRequest<P, R, S> logic) {
    Function<UserData, Route> apply = userData -> entity(unmarshaller(clazz), form -> handleExceptions(logic.apply(userData, p1, p2, form)));
  ...
  public <P, R, S, T, U> Route secured(P p1, R p2, S p3, T p4, Class<U> clazz, Parameter4WithBodyRequest<P, R, S, T, U> logic) {
	var unmarshaller = unmarshaller(clazz);
	Function<UserData, Route> apply = userData -> entity(unmarshaller, form -> handleExceptions(logic.apply(userData, p1, p2, p3, p4, form)));
	return withUserFromAuthHeader(apply);
  }
  ...
  private Route withUserFromAuthHeader(Function<UserData, Route> inner) {
    return optionalHeaderValueByName(
        ...
        maybeToken ->
            maybeToken
                .map((token) -> inner.apply(authentication.findUserByToken(token)))
