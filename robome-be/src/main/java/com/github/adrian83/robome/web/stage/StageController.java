...
public class StageController extends AllDirectives {
	...
	return route(
		get(new TwoParamsRoute(STAGES_PATH,
			(resourceOwnerId, tabId) -> security.secured(resourceOwnerId, tabId, this::getTableStages))),
		get(new ThreeParamsRoute(STAGE_PATH,
			(resourceOwnerId, tabId, stgId) -> security.secured(resourceOwnerId, tabId, stgId,
				this::getStageById))),
		delete(new ThreeParamsRoute(STAGE_PATH,
			(resourceOwnerId, tabId, stgId) -> security.secured(resourceOwnerId, tabId, stgId,
				this::deleteStage))),
		post(new TwoParamsAndFormRoute<NewStage>(STAGES_PATH, NewStage.class,
			(resourceOwnerId, tabId, clz) -> security.secured(resourceOwnerId, tabId, clz,
				this::persistStage))),
		put(new ThreeParamsAndFormRoute<UpdateStage>(STAGE_PATH, UpdateStage.class,
			(resourceOwnerId, tabId, stgId, clz) -> security.secured(resourceOwnerId, tabId, stgId, clz,
				this::updateStage))),
		options(new TwoParamsRoute(STAGES_PATH,
			(resourceOwnerId, tabId) -> complete(response.response200(GET, POST)))),
		options(new ThreeParamsRoute(STAGE_PATH,
			(resourceOwnerId, tabId, stgId) -> complete(response.response200(GET, DELETE, PUT)))));
    }
    ...
    private UpdateStageRequest toUpdateStageRequest(UserContext userCtx, String tableIdStr, String stageIdStr,
	    UpdateStage form) {

	return new UpdateStageRequest(form.title(),
