...
public class StageController extends AllDirectives {
    ...
    public Route createRoute() {
        return route(
                get(new TwoParamsRoute(STAGES_PATH, (resourceOwnerId, tabId) -> security.secured(resourceOwnerId, tabId, this::getTableStages))),
                get(new ThreeParamsRoute(STAGE_PATH, (resourceOwnerId, tabId, stgId) -> security.secured(resourceOwnerId, tabId, stgId, this::getStageById))),
                delete(new ThreeParamsRoute(STAGE_PATH, (resourceOwnerId, tabId, stgId) -> security.secured(resourceOwnerId, tabId, stgId, this::deleteStage))),
                post(new TwoParamsAndFormRoute<>(STAGES_PATH, NewStage.class, (resourceOwnerId, tabId, clz) -> security.secured(resourceOwnerId, tabId, clz, this::persistStage))),
                put(new ThreeParamsAndFormRoute<>(STAGE_PATH, UpdateStage.class, (resourceOwnerId, tabId, stgId, clz) -> security.secured(resourceOwnerId, tabId, stgId, clz, this::updateStage))),
                options(new TwoParamsRoute(STAGES_PATH, (resourceOwnerId, tabId) -> complete(response.response200(GET, POST)))),
                options(new ThreeParamsRoute(STAGE_PATH, (resourceOwnerId, tabId, stgId) -> complete(response.response200(GET, DELETE, PUT)))));
    }

    private CompletionStage<HttpResponse> persistStage(UserData user, String resourceOwnerIdStr, String tableIdStr,
            NewStage form) {
    ...
    private CompletionStage<HttpResponse> updateStage(UserData user, String resourceOwnerIdStr, String tableIdStr,
            String stageIdStr, UpdateStage form) {
    ...
    private CompletionStage<HttpResponse> deleteStage(UserData user, String resourceOwnerIdStr, String tableIdStr,
            String stageIdStr) {
    ...
    private CompletionStage<HttpResponse> getStageById(UserData user, String resourceOwnerIdStr, String tableIdStr,
            String stageIdStr) {
    ...
    private CompletionStage<HttpResponse> getTableStages(UserData user, String resourceOwnerIdStr, String tableIdStr) {
    ...
    private ListTableStagesQuery toListTableStagesRequest(UserContext userCtx, String tableIdStr) {
        return new ListTableStagesQuery(TableKey.create(userCtx.resourceOwnerIdOrError(), tableIdStr));
    }

    private DeleteStageCommand toDeleteStageRequest(UserContext userCtx, String tableIdStr, String stageIdStr) {
        return new DeleteStageCommand(StageKey.create(userCtx.resourceOwnerIdOrError(), tableIdStr, stageIdStr));
    }

    private UpdateStageCommand toUpdateStageRequest(UserContext userCtx, String tableIdStr, String stageIdStr, UpdateStage form) {
        return new UpdateStageCommand(form.title(), StageKey.create(userCtx.resourceOwnerIdOrError(), tableIdStr, stageIdStr));
    }

    private NewStageCommand toNewStageRequest(UserContext userCtx, String tableIdStr, NewStage form) {
        return new NewStageCommand(form.title(), userCtx.resourceOwnerIdOrError(),
                TableKey.create(userCtx.resourceOwnerIdOrError(), tableIdStr));
    }

    private GetStageQuery toGetStageRequest(UserContext userCtx, String tableIdStr, String stageIdStr) {
        return new GetStageQuery(StageKey.create(userCtx.resourceOwnerIdOrError(), tableIdStr, stageIdStr));
    }
}
