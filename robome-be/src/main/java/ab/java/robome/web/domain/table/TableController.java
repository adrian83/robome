...
public class TableController extends AbstractController {
	...
	@Inject
	public TableController(TableService tableService, SecurityUtils securityUtils, Config config,
	...
	public Route createRoute() {
		return route(options(() -> pathPrefix(TABLES, () -> pathEndOrSingleSlash(this::handleCreateTableOptions))),
				options(() -> pathPrefix(TABLES,
						() -> pathPrefix(segment(), tableId -> pathEndOrSingleSlash(this::handleUpdateTableOptions)))),

				get(() -> pathPrefix(TABLES,
						() -> pathEndOrSingleSlash(() -> optionalHeaderValueByName(HttpHeader.AUTHORIZATION.getText(),
								jwtToken -> securityUtils.authorized(jwtToken, userData -> getTables(userData)))))),

				get(() -> pathPrefix(TABLES,
						() -> pathPrefix(segment(),
								tableId -> pathEndOrSingleSlash(
				...
				put(() -> pathPrefix(TABLES, () -> pathPrefix(segment(), tableId -> pathEndOrSingleSlash(
						() -> optionalHeaderValueByName(HttpHeader.AUTHORIZATION.getText(), jwtToken -> securityUtils
								.authorized(jwtToken, userData -> entity(Jackson.unmarshaller(NewTable.class),
										updatedTable -> this.updateTable(tableId, updatedTable, userData)))))))),

				post(() -> path(TABLES,
						() -> pathEndOrSingleSlash(() -> optionalHeaderValueByName(HttpHeader.AUTHORIZATION.getText(),
								jwtToken -> securityUtils.authorized(jwtToken,
										userData -> entity(Jackson.unmarshaller(NewTable.class),
												newTable -> this.persistTable(newTable, userData))))))));
	}
	...
	private Route updateTable(String tableIdStr, NewTable newTable, UserData userData) {
		...
		CompletionStage<Optional<Done>> futureMaybeTable = tableService.getTable(userData.getId(), id)
				.thenApply(maybeTable -> maybeTable.map(table -> {
					Table updatedTable = new Table(tableId, userData.getId(), newTable.getTitle(),
	...
	private Route persistTable(NewTable newTable, UserData userData) {
		...
		Table table = new Table(tableId, userData.getId(), newTable.getTitle(), newTable.getDescription(),
				TableState.ACTIVE, utcNow, utcNow);
